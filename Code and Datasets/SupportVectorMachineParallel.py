...
class SupportVectorMachine(object):
    ...
    def diabetes(self):
        ...
        # End of data implementations  
        
        # Thread function to create threads for each dataset to be run through the svm
        def threadSVMaster(self, threadCount):
            functionList = [self.covid19(), self.breastCancer(), self.diabetes(),\
                            self.kidneyDisease(), self.heartDisease()]
            threadList = []
            for i in range(threadCount):
                 thread = th.Thread(target=functionList[i])
...
# MPI Function to split dataset between cores vs using threads
def mpiMaster(svm):
    comm = MPI.COMM_WORLD
    rank = comm.Get_rank()    
    ...
    if rank == 0:
        for i in range(1, 6):
            timesList.append(comm.recv(source=i, tag=i))        
        totalTime = max(timesList)    
        print("The total runtime for all of the datasets was {} (in seconds) and {} (in minutes)".format(totalTime, totalTime/60))
    elif rank == 1:
        svm.breastCancer()
        endTime = ti.default_timer()            
        totalTime = (endTime - startTime) # in seconds
        comm.send(totalTime, dest=0, tag=1)
    elif rank == 2:
        svm.diabetes()
        endTime = ti.default_timer()            
        totalTime = (endTime - startTime) # in seconds
        comm.send(totalTime, dest=0, tag=2)
    elif rank == 3:
        svm.kidneyDisease()
        endTime = ti.default_timer()            
        totalTime = (endTime - startTime) # in seconds
        comm.send(totalTime, dest=0, tag=3)
    elif rank == 4:
        svm.heartDisease()
        endTime = ti.default_timer()            
        totalTime = (endTime - startTime) # in seconds
        comm.send(totalTime, dest=0, tag=4)
    elif rank == 5:
        svm.covid19()
        endTime = ti.default_timer()            
        totalTime = (endTime - startTime) # in seconds
        comm.send(totalTime, dest=0, tag=5)

learningRate = 0.0001
constant = 10000
epochs = 16384
svm = SupportVectorMachine(learningRate, constant, epochs)
